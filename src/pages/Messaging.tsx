        ...
        <div className="p-4 border-b border-slate-100">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/10 transition-all"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filteredFaculty.map((fac) => (
            <div key={fac.id} onClick={() => { setActiveChat(fac); setIsUnlocked(false); setPassphrase(''); }} className={cn(
              ...
              activeChat?.id === fac.id ? "bg-blue-50/30 border-blue-500" : "border-transparent"
            )}>
          ...
          <>
            <div className="h-16 border-b border-slate-100 bg-white px-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-xs ring-2 ring-white">
                  {activeChat.name[0]}
                </div>
                <div>
              ...
              <div className="flex items-center gap-4 text-slate-400">
                <Phone className="w-4 h-4 hover:text-blue-600 cursor-pointer" />
                <Video className="w-4 h-4 hover:text-blue-600 cursor-pointer" />
                <MoreVertical className="w-4 h-4 hover:text-blue-600 cursor-pointer" />
              </div>
            </div>
                        ...
                        <div className={cn(
                          ...
                          {msg.isEncrypted && (
                            <ShieldCheck className={cn(
                              ...
                              msg.senderId === user?.uid ? "text-blue-400" : "text-slate-300"
                            )} />
                          )}
                        </div>
