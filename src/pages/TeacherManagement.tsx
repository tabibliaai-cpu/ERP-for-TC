...
function ScoreBar({ label, score, max = 10, color = 'bg-indigo-500' }: { label: string; score?: number; max?: number; color?: string }) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600">Theological Institution</span>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/60 flex flex-wrap items-center gap-3 shadow-sm">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input type="text" placeholder="Search by name, email, ID, dept..." value={searchTerm}
            ...
            className="w-full pl-11 pr-4 py-3 bg-slate-50/50 border-none rounded-xl text-sm outline-none focus:ring-4 focus:ring-indigo-100 transition-all placeholder:text-slate-300 font-medium" />
        </div>
                  ...
                  <tr key={teacher.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div className="w-11 h-11 overflow-hidden rounded-xl bg-slate-100 flex items-center justify-center font-bold text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500 shadow-sm uppercase text-xs">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 items-center mt-0.5">
                            {teacher.facultyId && <span className="text-[8px] uppercase font-black tracking-widest text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">{teacher.facultyId}</span>}
            ...
            <div className="p-8 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-2xl overflow-hidden bg-indigo-50 border-2 border-indigo-100 flex items-center justify-center shadow-xl shadow-indigo-100/20">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      {selectedTeacher.facultyId && <span className="text-[9px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded uppercase tracking-widest">{selectedTeacher.facultyId}</span>}
              ...
              {isAdmin && (
                <div className="flex items-center gap-3 mt-6 pt-6 border-t border-slate-100">
                  <a href={`/teacher-enrollment?edit=${selectedTeacher.id}`} className="px-4 py-2 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-colors flex items-center gap-2 no-underline">
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><Phone className="w-4 h-4 text-indigo-500" /> Contact Information</h3>
          {[
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 space-y-4">
            <ScoreBar label="Teaching Quality" score={teacher.performance?.teachingQualityScore} color="bg-indigo-500" />
          ...
          <div className="space-y-3">
            {data.subjects.map(s => (
              <div key={s.id} className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-100 hover:shadow-sm transition-shadow">
                ...
                <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase">{s.teacherIds?.includes(teacher.id!) ? 'Teacher' : 'Moderator'}</span>
              </div>
            ))}
          </div>
        </div>
...
function DocumentsTab({ teacher }: { teacher: Faculty }) {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><FolderOpen className="w-4 h-4 text-indigo-500" /> Documents</h3>
      {docs ? (
          ...
          {[
            ...
            <div key={item.label} className={cn("p-5 rounded-2xl border transition-all hover:shadow-sm",
              ...
              {(item.url || (item.urls && item.urls.length > 0)) ? (
                <div className="flex flex-wrap gap-2">
                  {item.url && <span className="px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase tracking-wider cursor-pointer hover:bg-indigo-100">View</span>}
                  {item.urls?.map((u, i) => <span key={i} className="px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase tracking-wider cursor-pointer hover:bg-indigo-100">Doc {i + 1}</span>)}
                </div>
              ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <ScoreBar label="Teaching Quality" score={p?.teachingQualityScore} color="bg-indigo-500" />
          ...
          <div className="space-y-3">
            {data.reviews.map((r: TeacherPerformanceReview) => (
              <div key={r.id} className="p-5 bg-slate-50/50 rounded-2xl border border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase">{r.type} Review</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                  <div className="bg-white p-3 rounded-xl"><p className="text-[8px] font-black text-slate-400 uppercase">Teaching</p><p className="text-sm font-bold text-indigo-600">{r.teachingQualityScore || 0}/10</p></div>
            ...
            <div key={s.id} className="p-5 bg-white rounded-2xl border border-slate-100 hover:shadow-sm transition-shadow">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  {s.videoUrl && <Video className="w-4 h-4 text-indigo-400" />}
...
function StudentsTab({ teacher, data }: { teacher: Faculty; data: any }) {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><GraduationCap className="w-4 h-4 text-indigo-500" /> Students</h3>
      <div className="bg-white p-6 rounded-2xl border border-slate-100">
        ...
        <div className="space-y-3">
          {data.subjects.map((s: Subject) => (
            <div key={s.id} className="p-4 bg-slate-50/50 rounded-xl border border-slate-100 flex items-center justify-between">
              ...
              <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black">{s.studentIds?.length || 0} Students</span>
            </div>
          ))}
        </div>
      ) : (
