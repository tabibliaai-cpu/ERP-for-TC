...
import {
  ...
  CheckCircle, Clock, Download, Star, Award, TrendingUp, Coffee, AlertCircle
} from 'lucide-react';
import { getTeachers, createTeacher, getTeacher, updateTeacher, getToken } from '../../utils/api';
...
function mapTeacherFromApi(raw: any): Teacher {
  return {
    ...
    yearsInMinistry: raw.years_in_ministry || raw.yearsInMinistry || '',
    spiritualGifts: raw.spiritual_gifts || raw.spiritualGifts || '',
    testimony: raw.testimony || '',
    statementOfFaith: raw.statement_of_faith || raw.statementOfFaith || '',
    highestQual: raw.highest_qual || raw.highestQual || '',
    theologicalDegree: raw.theological_degree || raw.theologicalDegree || '',
    seminary: raw.seminary || '',
    degreeYear: raw.degree_year || raw.degreeYear || '',
    certifications: raw.certifications || '',
    specialization: raw.specialization || '',
    role: raw.role || '',
    department: raw.department || '',
    subjectsAssigned: raw.subjects_assigned || raw.subjectsAssigned || '',
    dateOfJoining: raw.date_of_joining || raw.dateOfJoining || '',
    employmentType: raw.employment_type || raw.employmentType || '',
    experience: raw.experience || '',
    assignedCourses: raw.assigned_courses || raw.assignedCourses || '',
    classBatch: raw.class_batch || raw.classBatch || '',
    weeklySchedule: raw.weekly_schedule || raw.weeklySchedule || '',
    lectureHours: raw.lecture_hours || raw.lectureHours || '',
    teachingMode: raw.teaching_mode || raw.teachingMode || '',
    callingType: raw.calling_type || raw.callingType || '',
    ministryExperience: raw.ministry_experience || raw.ministryExperience || '',
    currentMinistryRole: raw.current_ministry_role || raw.currentMinistryRole || '',
    churchLeadership: raw.church_leadership || raw.churchLeadership || '',
    fieldExperience: raw.field_experience || raw.fieldExperience || '',
    salaryStructure: raw.salary_structure || raw.salaryStructure || '',
    bankDetails: raw.bank_details || raw.bankDetails || '',
    paymentFrequency: raw.payment_frequency || raw.paymentFrequency || '',
    allowances: raw.allowances || '',
    deductions: raw.deductions || '',
    idProof: raw.id_proof || raw.idProof || '',
    certificates: raw.certificates || '',
    experienceLetters: raw.experience_letters || raw.experienceLetters || '',
    ordinationCert: raw.ordination_cert || raw.ordinationCert || '',
    recommendations: raw.recommendations || '',
    cv: raw.cv || '',
    studentFeedback: raw.student_feedback || raw.studentFeedback || '',
    academicMetrics: raw.academic_metrics || raw.academicMetrics || '',
    spiritualLeadership: raw.spiritual_leadership || raw.spiritualLeadership || '',
    attendance: raw.attendance || '',
    adminRatings: raw.admin_ratings || raw.adminRatings || '',
...
export default function TeachersPage() {
  ...
  const fetchTeachers = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setTeachers(data.map(mapTeacherFromApi));
      }
    } catch (err) {
      console.warn('Failed to fetch teachers, using fallback data', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setToast({ message: err?.message || 'Failed to create faculty', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
                ...
                <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-right">
                    <button onClick={() => {
                      const token = getToken();
                      if (token) {
                        getTeacher(t.id).then((data) => { setViewTeacher(mapTeacherFromApi(data)); setViewTab(0); }).catch(() => { setViewTeacher(t); setViewTab(0); });
                      } else { setViewTeacher(t); setViewTab(0); }
