...
function statusBadge(status: string) {
  const map: Record<string, { bg: string; text: string; border: string }> = {
    ...
    refunded: { bg: 'bg-indigo-50', text: 'text-indigo-600', border: 'border-indigo-100' },
...
export function Finance() {
  ...
  const EmptyState = ({ icon: Icon, message, actionLabel, onAction }: { icon: React.ElementType; message: string; actionLabel?: string; onAction?: () => void }) => (
    <div className="flex flex-col items-center justify-center h-80 text-center p-6 text-slate-400">
      ...
      {actionLabel && onAction && (
        <button onClick={onAction} className="mt-6 px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all">
          {actionLabel}
        </button>
      )}
    </div>
  ...
  const FormInput = (props: React.InputHTMLAttributes<HTMLInputElement>) => (
    <input
      {...props}
      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-100 transition-all outline-none text-sm font-medium"
    />
  );
  ...
  const FormSelect = (props: React.SelectHTMLAttributes<HTMLSelectElement>) => (
    <select
      {...props}
      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-100 transition-all outline-none text-sm font-bold"
    />
  );
    ...
    <div className="space-y-1 relative">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-100 transition-all outline-none text-sm font-medium"
        />
      </div>
      <AnimatePresence>
        {studentDropdownOpen && (
    ...
    <div className="pt-4 flex gap-3">
      ...
      <button
        ...
        className={cn(
          ...
          submitColor === 'dark' ? 'bg-slate-900 hover:bg-slate-800 shadow-slate-200' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-100',
        )}
      >
        {submitLabel}
      </button>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {[
          ...
          { label: 'This Month Collection', value: monthlyCollection, icon: TrendingUp, color: 'text-indigo-600', bg: 'bg-indigo-50', isCurrency: true },
          ...
          <div key={stat.label} className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm group hover:border-indigo-100 transition-all">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button onClick={() => { setActiveTab('fee-structures'); setTimeout(() => openCreateFeeStructure(), 100); }} className="px-6 py-3 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-all uppercase tracking-widest flex items-center gap-2 shadow-xl shadow-indigo-100">
  ...
  const renderFeeStructures = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
          <input type="text" placeholder="Search fee structures..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-4 focus:ring-indigo-100 transition-all outline-none w-64" />
        </div>
                ...
                {feeStructures.filter((fs) => !searchQuery || fs.name.toLowerCase().includes(searchQuery.toLowerCase()) || fs.program.toLowerCase().includes(searchQuery.toLowerCase())).map((fs) => (
                  <tr key={fs.id} className="hover:bg-slate-50/50 transition-colors group text-sm">
                    ...
                    <td className="px-8 py-6 text-right flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEditFeeStructure(fs)} className="p-2 rounded-xl hover:bg-indigo-50 transition-all"><Edit3 className="w-4 h-4 text-indigo-500" /></button>
  ...
  const renderStudentFees = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
          <input type="text" placeholder="Search student fees..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-4 focus:ring-indigo-100 transition-all outline-none w-64" />
        </div>
  ...
  const renderPayments = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
          <input type="text" placeholder="Search payments..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-4 focus:ring-indigo-100 transition-all outline-none w-64" />
        </div>
                ...
                {filteredPayments.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group text-sm">
                    ...
                    <td className="px-6 py-5 text-right flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => printReceipt(p)} className="p-2 rounded-xl hover:bg-indigo-50 transition-all"><Printer className="w-4 h-4 text-indigo-500" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  const renderInvoices = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
          <input type="text" placeholder="Search invoices..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-4 focus:ring-indigo-100 transition-all outline-none w-64" />
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          ...
          { label: 'Fee Assignments', value: studentFees.length, icon: FileText, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
          ...
          <button onClick={exportReportsCSV} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center gap-2">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {scholarships.map((sc) => (
                        <tr key={sc.id} className="hover:bg-slate-50/50 transition-colors group text-sm">
                          ...
                          <td className="px-6 py-6 text-center font-mono font-bold text-indigo-600 tabular-nums">
      ...
      <form onSubmit={handleSaveFeeStructure} className="space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addFeeComponent} className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-100 transition-all flex items-center gap-1.5">
              ...
              <motion.div key={index} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-slate-50 rounded-2xl p-4 space-y-3 border border-slate-100">
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={comp.isMandatory} onChange={(e) => updateFeeComponent(index, 'isMandatory', e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
          ...
          {fsForm.components.length > 0 && (
            <div className="flex items-center justify-between p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400">Auto-calculated Total</span>
              <span className="text-xl font-mono font-bold text-indigo-600 tabular-nums">{formatCurrency(fsTotal)}</span>
            </div>
          )}
        </div>
      ...
      <form onSubmit={handleAssignFee} className="space-y-5">
        ...
        {selectedStructure && (
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
            ...
            <div className="flex justify-between text-xs pt-2 border-t border-slate-200"><span className="text-slate-400 font-bold">Adjusted Fee:</span><span className="font-mono font-bold text-indigo-600">{formatCurrency(sfAdjustedFee)}</span></div>
          </div>
        )}
      ...
      <form onSubmit={handleRecordPayment} className="space-y-5">
        ...
        {pmForm.studentFeeId && pmForm.amount > 0 && (
          <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400">Generated IDs</span>
            <div className="flex gap-3">
              <span className="text-[10px] font-mono text-indigo-600 bg-white px-2 py-1 rounded-md">PAY-{new Date().getFullYear()}-XXXXX</span>
              <span className="text-[10px] font-mono text-indigo-600 bg-white px-2 py-1 rounded-md">RCPT-{new Date().getFullYear()}-XXXXX</span>
            </div>
          </div>
        )}
